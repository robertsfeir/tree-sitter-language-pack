...
// alef:hash:8d426d126c46b4e95a60516bc4f721b8327d58e018f28624cdbbe4de461df68c
...
$repoRoot = __DIR__ . '/../../';
$localExtCandidates = [
    $repoRoot . 'target/release/libts_pack_core_php' . $extSuffix,
    $repoRoot . 'crates/ts-pack-core-php/target/release/libts_pack_core_php' . $extSuffix,
];
...
if (!file_exists($extPath)) {
    fwrite(STDERR, "error: no tree_sitter_language_pack PHP extension build found.\n");
    foreach ($localExtCandidates as $candidate) {
    ...
    fwrite(STDERR, "  cargo build --release -p ts-pack-core-php\n");
    exit(1);
}
...
$php = PHP_BINARY;
$isolatedIni = alef_build_isolated_ini($php, 'tree_sitter_language_pack');
...
$process = proc_open(
    ...
    $pipes,
    null,
    ['ALEF_PREFLIGHT_EXT_NAME' => 'tree_sitter_language_pack'] + $_ENV,
    ['bypass_shell' => true]
);
if ($process === false) {
    fwrite(STDERR, "error: failed to spawn PHP preflight check for the tree_sitter_language_pack extension.\n");
    exit(1);
}
...
if (stripos($preflightStderr, 'already loaded') !== false) {
    fwrite(STDERR, "error: the tree_sitter_language_pack extension is already loaded from an ambient php.ini\n");
...
if ($preflightExit !== 0 || $preflightStdout === '') {
    fwrite(STDERR, "error: the tree_sitter_language_pack extension did not load from $extPath.\n");
...
if ($loadedVersion !== '1.17.0') {
    fwrite(STDERR, "error: loaded tree_sitter_language_pack extension version mismatch.\n");
    fwrite(STDERR, "  extension at $extPath reports version: $loadedVersion\n");
    fwrite(STDERR, "  expected version: 1.17.0\n");
