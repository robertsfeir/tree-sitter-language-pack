...
// alef:hash:ced2b36d862fbabde7d5944581ff04acd241b50e66d290586b3df4997267d9b9
    ...
    pub fn new(opts: std::collections::HashMap<String, rustler::Term>) -> Self {
        Self {
            ...
            data_extraction: opts
                ...
                .and_then(|t| t.decode().ok())
                .unwrap_or(false),
            max_source_bytes: opts.get("max_source_bytes").and_then(|t| t.decode().ok()),
            parse_timeout_ms: opts.get("parse_timeout_ms").and_then(|t| t.decode().ok()),
        }
    }
}
...
pub fn configure(config: Option<String>) -> Result<(), String> {
    let config_core: Option<tree_sitter_language_pack::PackConfig> = config
        ...
        .transpose()
        .map_err(|error| format!("failed to deserialize parameter `config` for `configure`: {error}"))?;
...
pub fn init(config: Option<String>) -> Result<(), String> {
    let config_core: Option<tree_sitter_language_pack::PackConfig> = config
        .map(|s| serde_json::from_str::<tree_sitter_language_pack::PackConfig>(&s))
        .transpose()
        .map_err(|error| format!("failed to deserialize parameter `config` for `init`: {error}"))?;
...
/// ```
#[rustler::nif]
pub fn manifest_groups() -> Result<Vec<String>, String> {
    let result = tree_sitter_language_pack::manifest_groups().map_err(|e| e.to_string())?;
    Ok(result)
}
...
pub fn process(source: String, config: Option<String>) -> Result<ProcessResult, String> {
    let config_core: Option<tree_sitter_language_pack::ProcessConfig> = config
        ...
        .transpose()
        .map_err(|error| format!("failed to deserialize parameter `config` for `process`: {error}"))?;
...
pub fn languageregistry_process(
    ...
    let config_core: Option<tree_sitter_language_pack::ProcessConfig> = config
        .map(|s| serde_json::from_str::<tree_sitter_language_pack::ProcessConfig>(&s))
        .transpose()
        .map_err(|error| format!("failed to deserialize parameter `config` for `languageregistry_process`: {error}"))?;
    let result = resource
        .inner
        .read()
        .unwrap_or_else(|e| e.into_inner())
...
#[rustler::nif]
pub fn parser_set_max_source_bytes(resource: rustler::ResourceArc<Parser>, max_bytes: Option<usize>) -> () {
    resource
        .inner
        .write()
        .unwrap_or_else(|e| e.into_inner())
        .set_max_source_bytes(max_bytes)
}
...
#[rustler::nif]
pub fn parser_set_parse_timeout_ms(resource: rustler::ResourceArc<Parser>, timeout_ms: Option<u64>) -> () {
    resource
        .inner
        .write()
        .unwrap_or_else(|e| e.into_inner())
        .set_parse_timeout_ms(timeout_ms)
}
...
#[rustler::nif]
pub fn processconfig_with_max_source_bytes(obj: ProcessConfig, max_bytes: Option<usize>) -> ProcessConfig {
    tree_sitter_language_pack::ProcessConfig::from(obj)
        .with_max_source_bytes(max_bytes)
        .into()
}
...
#[rustler::nif]
pub fn processconfig_with_parse_timeout_ms(obj: ProcessConfig, timeout_ms: Option<u64>) -> ProcessConfig {
    tree_sitter_language_pack::ProcessConfig::from(obj)
        .with_parse_timeout_ms(timeout_ms)
        .into()
}
...
#[rustler::nif]
pub fn processconfig_validate(obj: String) -> Result<(), String> {
    let obj: tree_sitter_language_pack::ProcessConfig =
        serde_json::from_str::<tree_sitter_language_pack::ProcessConfig>(&obj)
            .map_err(|error| format!("failed to deserialize parameter `obj` for `processconfig_validate`: {error}"))?;
