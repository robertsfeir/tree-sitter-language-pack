...
// alef:hash:7261409f3969d3364f7970a837ee2a692f076900f2d07c9e47f5ad665f23ad5a
...
dependencies {
    ...
    api("io.github.tree-sitter:ktreesitter:0.25.1")
...
tasks.register("buildHostJni", Exec::class) {
    if (project.providers.gradleProperty("alef.skipHostJni").orNull != "true") {
        ...
        val jniManifest = configuredManifest?.let(::file) ?: generateSequence(projectDir) { it.parentFile }
            .map { it.resolve("crates/tree-sitter-language-pack-jni/Cargo.toml") }
            .firstOrNull { it.isFile }
            ?: throw GradleException(
                "Cannot locate crates/tree-sitter-language-pack-jni/Cargo.toml from $projectDir; " +
...
tasks.register("copyHostJni", Copy::class) {
    if (project.providers.gradleProperty("alef.skipHostJni").orNull != "true") {
        ...
        val jniManifest = configuredManifest?.let(::file) ?: generateSequence(projectDir) { it.parentFile }
            .map { it.resolve("crates/tree-sitter-language-pack-jni/Cargo.toml") }
            .firstOrNull { it.isFile }
            ?: throw GradleException("Cannot locate crates/tree-sitter-language-pack-jni/Cargo.toml from $projectDir")
