...
// `Node` is a zero-copy view: slice your own source bytes with `byte_range()`.
let source = "def add(a, b):\n    return a + b";
let name_range = name.byte_range();
let params_range = params.byte_range();
println!("{}", &source[name_range.start..name_range.end]);     // "add"
println!("{}", &source[params_range.start..params_range.end]); // "(a, b)"
# Ok::<(), Box<dyn std::error::Error>>(())
```
...
Named nodes carry semantic meaning (`identifier`, `call_expression`, `string`). Anonymous nodes are punctuation and keywords (`(`, `)`, `def`, `:`). Iterate `named_child(i)` over `named_child_count()` when you want semantic nodes and no punctuation tokens.

```rust
for i in 0..root.named_child_count() {
    let child = root.named_child(i as u32).ok_or("missing named child")?;
    println!("{}", child.kind());
}
# Ok::<(), Box<dyn std::error::Error>>(())
```
...
| `byte_range()`        | `ByteRange { start, end }` — slice your own source   |
| `named_child(i)`      | The i-th named child, or `None`                      |
